...
static KeySet * embeddedSpec (void)
{
	...
	keyNew ("/fade", KEY_META, "default", "1", KEY_META, "description", "Whether transitions between color temperatures (e.g. from daytime to nighttime) should happen fast or slow.", KEY_META, "example", "0", KEY_META, "opt", "f", KEY_META, "opt/arg", "none", KEY_META, "opt/long", "fade", KEY_META, "type", "boolean", KEY_END),
...
/**
 * Initializes an instance of Elektra for the application '/sw/redshift/#0/current'.
 *
				 ...
				 ElektraError ** error)
{
	...
	KS_END);
;
	Key * parentKey = keyNew ("/sw/redshift/#0/current", KEY_END);
	...
	Elektra * e = elektraOpen ("/sw/redshift/#0/current", defaults, contract, error);
...
void exitForSpecload (int argc, const char * const * argv)
{
	...
	Key * parentKey = keyNew ("spec:/sw/redshift/#0/current", KEY_META, "system:/elektra/quickdump/noparent", "", KEY_END);
