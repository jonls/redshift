...
/**
 * Get the value of key 'fade' (tag #ELEKTRA_TAG_FADE).
 *
 * @param elektra Instance of Elektra. Create with loadConfiguration().

 *
 * @return the value of 'fade'.

 */// 
static inline kdb_boolean_t ELEKTRA_GET (ELEKTRA_TAG_FADE) (Elektra * elektra )
{
	
	return ELEKTRA_GET (Boolean) (elektra, "fade");
}


/**
 * Set the value of key 'fade' (tag #ELEKTRA_TAG_FADE).
 *
 * @param elektra Instance of Elektra. Create with loadConfiguration().
 * @param value   The value of 'fade'.

 * @param error   Pass a reference to an ElektraError pointer.
 *                Will only be set in case of an error.
 */// 
static inline void ELEKTRA_SET (ELEKTRA_TAG_FADE) (Elektra * elektra,
						      kdb_boolean_t value,  ElektraError ** error)
{
	
	ELEKTRA_SET (Boolean) (elektra, "fade", value, error);
}
